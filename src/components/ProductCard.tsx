...
const ProductCard = memo(function ProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      <Card className="h-full flex flex-col overflow-hidden bg-card/80 backdrop-blur supports-[backdrop-filter]:bg-card/60 border-border/60 hover:shadow-lg transition-shadow">
          ...
          <DialogTrigger asChild>
            <div className="cursor-pointer group">
              {product.image_url ? (
                ...
                <div className="h-full w-full bg-linear-to-br from-muted/50 to-muted/20 flex flex-col items-center justify-center gap-2 text-primary">
                  <div className="font-display text-2xl font-black tracking-tighter animate-pulse opacity-40 italic">قريباً..</div>
              ...
              <div className="flex-1 bg-muted flex items-center justify-center p-4">
                {activeImage ? (
                  ...
                  <div className="text-center space-y-3">
                    <div className="font-display text-4xl font-black text-primary/10 italic">قريباً..</div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-50">جاري تجهيز صورة المنتج</p>
                  </div>
                )}
              </div>
      ...
      <CardFooter className="mt-auto p-3 pt-0 flex flex-col gap-3">
        {inCart ? (
