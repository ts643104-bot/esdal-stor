  ...
  const handleCheckout = () => {
    ...
    import("@/lib/db").then(async ({ db }) => {
      ...
      const orderId = await db.addOrder(
        ...
        totalDiscount, dateStr, user?.uid, pointsEarned, pointsToRedeem, receiptUrl
      );
      
      const msg = buildWhatsappMessage(cart.items, cart.totalPrice, shippingCost, totalDiscount, finalTotal, paymentMethod, senderPhone, customerName, customerAddress, customerPhone, governorate, `${shippingType === "express" ? "[Express] " : ""}${preferredTime}`, dateStr, orderId);
                            ...
                            onChange={async (e) => {
                              ...
                              if (file) {
                                ...
                                if (hasFirebase && storage) {
                                  ...
                                  try {
                                    const fileRef = ref(storage!, `receipts/${nanoid()}_${file.name}`);
                                  ...
                                  reader.onloadend = () => {
                                    ...
                                    toast.success("تم إرفاق الإيصال بنجاح (محلياً)");
                                  };
