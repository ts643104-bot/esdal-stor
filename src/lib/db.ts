...
const getLocalProducts = (): Product[] => {
  const stored = localStorage.getItem("hala_alyusr_products");
  if (stored) return JSON.parse(stored);
  
  const fb: Product[] = [
    {
      id: "hala-001",
      name: "عباءة استقبال راقية",
      price_egp: 450,
      image_url: "https://images.unsplash.com/photo-1736342182213-6c037467cb38?fm=jpg&q=60&w=1200&auto=format&fit=crop",
      category: "ملابس استقبال",
      in_stock: true,
      description: "خامة ممتازة وتصميم عصري مريح."
    },
    {
      id: "abaya-002",
  ...
  localStorage.setItem("hala_alyusr_products", JSON.stringify(fb));
  return fb;
};
...
export const db = {
  ...
  saveProducts: async (products: Product[]) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_products", JSON.stringify(products));
    }
  },
  deleteProduct: async (id: string) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_products", JSON.stringify(stored));
    }
  },
  getOrders: async (): Promise<Order[]> => {
    ...
    const stored = localStorage.getItem("hala_alyusr_orders");
  ...
  addOrder: async (
    ...
    userId?: string, loyaltyPointsEarned?: number, loyaltyPointsRedeemed?: number, paymentReceiptUrl?: string, note?: string
  ): Promise<string> => {
    ...
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_orders", JSON.stringify(orders));
    }
  ...
  updateOrderStatus: async (id: string, status: "pending" | "completed") => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_orders", JSON.stringify(updated));
    }
  },
  updateOrderStatusAndDeductStock: async (orderId: string, items: CartItem[]) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_orders", JSON.stringify(updatedOrders));
      ...
      localStorage.setItem("hala_alyusr_products", JSON.stringify(updatedProducts));
    }
  },
  clearOrders: async () => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_orders", "[]");
    }
  },
  getExpenses: async (): Promise<Expense[]> => {
    ...
    const stored = localStorage.getItem("hala_alyusr_expenses");
    return stored ? JSON.parse(stored) : [];
  },
  saveExpenses: async (expenses: Expense[]) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_expenses", JSON.stringify(expenses));
    }
  },
  deleteExpense: async (id: string) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_expenses", JSON.stringify(updated));
    }
  },
  getSettings: async (): Promise<StoreSettings> => {
    ...
    const stored = localStorage.getItem("hala_alyusr_settings");
  ...
  saveSettings: async (settings: StoreSettings) => {
    ...
    localStorage.setItem("hala_alyusr_settings", JSON.stringify(settings));
  },
  getUserProfile: async (id: string): Promise<UserProfile | null> => {
    ...
    const stored = localStorage.getItem("hala_alyusr_users");
  ...
  saveUserProfile: async (profile: UserProfile) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_users", JSON.stringify(updated));
    }
  },
  ...
  getPromoCodes: async (): Promise<PromoCode[]> => {
    ...
    const stored = localStorage.getItem("hala_alyusr_promo_codes");
    return stored ? JSON.parse(stored) : [];
  },
  savePromoCodes: async (codes: PromoCode[]) => {
    if (hasFirebase && dbFirestore) {
      ...
      localStorage.setItem("hala_alyusr_promo_codes", JSON.stringify(codes));
    }
  },
  deletePromoCode: async (id: string) => {
    if (hasFirebase && dbFirestore) {
      ...
      if (stored) {
        ...
        localStorage.setItem("hala_alyusr_promo_codes", JSON.stringify(codes.filter(c => c.id !== id)));
      }
    }
  }
};
